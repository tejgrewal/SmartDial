...
void Menu::tickAndDraw(TFT_eSprite &spr, ModulinoKnob &knob){
  int pos=knob.get(), d=pos-lastKnob;
  if(d!=0){
    Haptics::tap();
    int prev=menuIndex;
    if(d>0) menuIndex=(menuIndex+1)%COUNT; else menuIndex=(menuIndex-1+COUNT)%COUNT;
    if(menuIndex!=prev) startPop();
    lastKnob=pos;
  }

  spr.fillSprite(Theme::BG);
  const auto &it = ITEMS[menuIndex];
  float t = popT();
  const int iconCX=CX;
  const int iconCY = CY - 18 - int(8*(1.0f-t));

  switch(it.id){
    case MenuId::M_HOME:
      drawIconTransparent(spr, iconCX-60, iconCY-60, homeIcon, homeIcon_W, homeIcon_H, homeIcon_COLORKEY); break;
    case MenuId::M_BULB:
      drawIconTransparent(spr, iconCX-60, iconCY-60, bulbIcon, bulbIcon_W, bulbIcon_H, bulbIcon_COLORKEY); break;
    case MenuId::M_LAUNCHER:
    ...
    case MenuId::M_ABOUT:
      drawIconTransparent(spr, iconCX-60, iconCY-60, infoIcon, infoIcon_W, infoIcon_H, infoIcon_COLORKEY); break;
  }

  draw_label(spr, it.label);
  spr.pushSprite(0,0);
}
